...
def communication(connection,address):
    global CurrClientCount
    # increasing the client count with 1 and setting this as the value of the current Client
    ClientCountLock.acquire()
    CurrClientCount+=1
    ClientID=CurrClientCount
    ClientCountLock.release()
    while True:
        try:
            ...
            print(
                "Equation received from socket " + str(ClientID) +" ["
                + message
                + "] from "
                + address[0]
                + ":"
    ...
    # reducing the variable once the conneciton is closed
    ClientCountLock.acquire()
    CurrClientCount-=1
    ClientCountLock.release()
...
def main():
    ...
    while True:
        try:
            ...
            t = threading.Thread(target=communication, args=(connection, address,))
            t.start()
